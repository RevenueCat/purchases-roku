function IdentityTests(t)
    t.describe("Identity Tests", sub(t)
        ...
        t.it("Can check if user is anonymous", sub(t)
            result = Purchases().isAnonymous(sub(result, error)
                m.t.assert.isTrue(result, "Expected anonymous user id")
                m.t.assert.isInvalid(error, "Unexpected error")
            end sub)
            t.assert.isTrue(result, "Expected anonymous user id")
            t.assert.isTrue(Purchases().isAnonymous(), "Expected anonymous user id")
        end sub)

        t.it("Can retrieve app user id", sub(t)
            result = Purchases().appUserId(sub(result, error)
                m.t.assert.isInvalid(error, "Unexpected error")
                m.t.assert.isTrue(result.startsWith("$RCAnonymousID:"), "Expected anonymous user id")
            end sub)
            t.assert.isValid(result, "Unexpected result")
            t.assert.isValid(Purchases().appUserId(), "Unexpected result")
        end sub)
